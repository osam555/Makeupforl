...
/** 문항별 조회 — 서버(page.tsx)가 방문 기록에서 뽑아 넘긴다 */
export type QnaHit = { views: number; last: string | null }
          ...
          <div className="flex flex-col border-r border-[var(--a-e7ddd4)] bg-[var(--a-fcfaf8)]">
            ...
            <div className="max-h-[600px] flex-1 overflow-auto">
              {rows.map((x) => (
                ...
                >
                  <span
                    className="mt-0.5 min-w-[52px] text-[0.625rem] font-extrabold"
                  ...
                  <span className="flex-1">
                    ...
                    {/*
                      글자수 옆에 조회수와 마지막으로 읽힌 날(2026-09-13). 본문을 손볼 때
                      "이 문항을 누가 보긴 하나" 가 같이 보여야 어느 것부터 고칠지 정해진다.
                      기록을 남기기 시작한 뒤(2026-09-09~) 전체 합이다.
                    */}
                    {(() => {
                      const h = hits[x.slug]
                      return h && h.views > 0 ? (
                        <span
                          className="ml-1.5 inline-block text-[0.625rem] tabular-nums text-[var(--a-8a7b73)]"
                          title={`조회 ${h.views.toLocaleString()}회 · 마지막 ${h.last ?? '-'}`}
                        >
                          조회 <b className="text-[var(--a-3a322e)]">{h.views.toLocaleString()}</b>
                          {h.last && <> · {h.last.slice(5).replace('-', '.')}</>}
                        </span>
                      ) : (
                        <span className="ml-1.5 inline-block text-[0.625rem] text-[var(--a-c9bdb6)]">조회 0</span>
                      )
                    })()}
                  </span>
          ...
          {draft && (
            <div className="max-h-[720px] overflow-auto p-5 lg:p-6">
              ...
              >
                ...
                <span
                  className={`rounded px-2 py-0.5 text-[0.625rem] font-bold ${
                ...
                {dirty && (
                  <span className="rounded bg-amber-100 px-2 py-0.5 text-[0.625rem] font-bold text-amber-800">
                    수정됨 · 저장 필요
                  </span>
                )}
                {isStale(draft) && (
                  <span
                    className="rounded bg-amber-100 px-2 py-0.5 text-[0.625rem] font-bold text-amber-800"
                          ...
                          return (
                            <div key={ph.name} className="group relative">
                              ...
                              >
                                ...
                                {!on && mine && (
                                  <span className="absolute left-1 top-1 rounded bg-emerald-600 px-1 py-px text-[0.5625rem] font-extrabold text-white">
                                    NEW
                                  </span>
                                )}
                              </button>
