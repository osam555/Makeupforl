...
export async function POST(req: Request) {
  ...
  const list: (SeedItem & { title?: string; date?: string })[] =
    body?.mode === 'assets' ? ASSETS : body?.mode === 'reviews' ? REVIEWS : SEED
  ...
  for (const item of batch) {
    ...
    const dir =
      item.category === '_site' ? 'site' : item.category === '_review' ? 'reviews' : 'gallery'
