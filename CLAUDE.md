...
- 방문 기록(`/api/track`)은 사람을 식별하지 않는다. 쿠키·IP·UA 를 저장하지 않고 날짜별 문서 하나에
  ...
  봇(UA 에 bot·crawl·Yeti·headless 등)은 **판별만 하고 세지 않는다**(2026-09-18~) — Vercel 통계와
  숫자를 맞추기 위해서다. 그 전 기록(9/17 직접 방문 126건)은 크롤러가 섞여 있다.
  "직접 방문" 은 referrer 가 없는 전부다: 주소 직접 입력·즐겨찾기 외에 **카톡·문자·인스타 DM 링크**가
  여기 잡힌다(인앱 브라우저가 referrer 를 안 붙인다). 네이버·구글 유입 숫자는 Vercel 과 3~5% 안에서 맞고,
  차이는 세는 단위(Vercel 은 사람, 우리는 탭 세션) 때문이다.
